...
int main(int argc, char **argv) 
{
    ...
    ReadParas(data_path+"/config/fgi_para.yaml", config_setting);   

    // read TLS data
    pcl::PointCloud<pcl::PointXYZ>::Ptr tls_point_data(new pcl::PointCloud<pcl::PointXYZ>);
    std::vector<std::pair<Eigen::Vector3d, Eigen::Matrix3d>> gt_pose;
    ...
    for(int i=1; i<=StationNUM; i++)
    {
        ...
        readTLSData(data_path+"/data/fgi/"+ std::to_string(i) +".las", tls_point_data);    
        std::cout << "Read Target (" + std::to_string(i) +"), Points NUM: " << tls_point_data->size() << std::endl;
        
        std::pair<Eigen::Vector3d, Eigen::Matrix3d> trans;
        readFGITrans(data_path+"/data/fgi/"+ std::to_string(i) +".txt", trans);
        gt_pose.emplace_back(trans);
    ...
    for(int i=0; i<GTIN_map->pointMat_vec_.size(); i++)
    {
        ...
        cv::imwrite(data_path+"/data/fgi/" + std::to_string(i) + "-ori_fig.png", GTIN_map->pointMat_vec_[i]); 
        ...
        cv::imwrite(data_path+"/data/fgi/" + std::to_string(i) + "-filterd_fig.png", GTIN_map->pointMat_vec_[i]); 
    }
    ...
    for(int i=0; i<StationNUM; i++)
    {
        ...
        {
            currPos =  Eigen::Translation3d(gt_pose[i].first) * gt_pose[i].second;
            tlsTrans.push_back(currPos);
        }
    }
    ...
    write_error_vec(data_path+"/data/fgi/roughError.txt", rough_errors);
    write_pose(data_path+"/data/fgi/roughPoses.txt", tlsVec);
    ...
    write_error_vec(data_path+"/data/fgi/GTSAMError.txt", gtsam_errors);
    write_pose(data_path+"/data/fgi/GTSAMPoses.txt", optiTLSVec);
    ...
    for(int i=0; i<candidates_vec.size(); i++)
    {
        ...
        if(optiTLSVec[currID].isValued)
        {
            ...
            // writeLas(data_path+"/data/fgi/"+std::to_string(currID+1) +"-rough.las", ICP_target);
        ...
        for(int j=0; j<candidates_vec[i].candidateIDScore.size(); j++)
        {
            ...
            if(optiTLSVec[candID].isValued)
            {
                ...
                // writeLas(data_path+"/data/fgi/"+std::to_string(candID+1) +"-rough.las", ICP_source);
    ...
    write_error_vec(data_path+"/data/fgi/icpError.txt", ICP_errors);
    write_pose(data_path+"/data/fgi/icpPoses.txt", optiPosVec);
    ...
    write_error_vec(data_path+"/data/fgi/finalError.txt", Final_errors);
    write_pose(data_path+"/data/fgi/finalPoses.txt", finalTLSVec);

    // write the running time for each module
    std::ofstream outfile(data_path+"/data/fgi/time_stastic.txt");
